...
//#region Inicialización de la escena.
	function showBoxPlacement(){
		...
		initialBoxLenght = parseInt(document.getElementById("box-lenght").value);
		initialBoxWidth = parseInt(document.getElementById("box-width").value);
		initialBoxHeight = parseInt(document.getElementById("box-height").value);
		initialBoxMass = parseInt(document.getElementById("box-mass").value);
...
function changeLayer(){
	...
	if(oddLayer == true){
		...
		for (var i = 0; i < Object.keys(oddBoxes).length; i++){
		...
		for (var i = 0; i < Object.keys(evenBoxes).length; i++){
	...
	console.log(Object.keys(oddBoxes).length);
	console.log(Object.keys(evenBoxes).length)
...
function copyPreviousLayer(){
	...
	if(oddLayer == true){
		...
		for(var i = 0; i < evenBoxes.length; i++){
			...
			//Creación de la caja en la escena.
			spawnedBox = new THREE.Mesh(boxGeometry,boxTexture);
			spawnedBox.position.set(evenBoxes[i].position.x,evenBoxes[i].position.y,layerHeight);
			...
			//Introducción de los límites de la caja en arrays
			boundBottom.push(evenSizeY[i]/2);
			boundTop.push(evenSizeY[i]/2);
			boundLeft.push(evenSizeX[i]/2);
			boundRight.push(evenSizeX[i]/2);
		}
	}
